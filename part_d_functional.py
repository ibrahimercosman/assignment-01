...
def get_category_totals_functional(expense_list):
    ...
    return {
        c: sum(e["amount"] for e in expense_list if e["category"] == c)
        for c in categories
    }
...
def format_expenses(expense_list):
    ...
    return list(
        map(
            lambda e: f'{e["date"]} | {e["category"]} | {e["description"]} | ${e["amount"]:.2f}',
            expense_list,
        )
    )
...
if __name__ == "__main__":
    from part_c_procedural import get_total, get_category_totals, get_above_average
